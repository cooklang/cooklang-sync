...
Run: `cargo clippy -p cooklang-sync-server --tests -- -D warnings`
Expected: clean.
...
> **Why not `--all-features`:** the server crate defines two mutually-exclusive features (`database_sqlite` and `database_postgres`) gated by a `compile_error!` in `server/src/lib.rs`. `--all-features` enables both and fails to compile. Run client and server clippy/test separately (see below).

- [ ] **Step 2: Clippy gate**

Run, in parallel:
- `cargo clippy -p cooklang-sync-client --tests -- -D warnings`
- `cargo clippy -p cooklang-sync-server --tests -- -D warnings`

Expected: both clean. No new warnings anywhere.
